...
//该声明使得stl容器包含相应eigen结构时不会报错
EIGEN_DEFINE_STL_VECTOR_SPECIALIZATION(Eigen::Vector3d)
EIGEN_DEFINE_STL_VECTOR_SPECIALIZATION(Eigen::Vector2d)
#endif

//new win10 cmd version does not support ansi color
//#define SVO_INFO_STREAM(x) std::cerr<<"\033[0;0m[INFO] "<<x<<"\033[0;0m"<<std::endl;
#define SVO_INFO_STREAM(x) std::cerr<<"*****[INFO]"<<x<<"*****"<<std::endl;
...
#define SVO_WARN_STREAM(x) std::cerr<<"*****[WARN] "<<x<<"*****"<<std::endl;
//#define SVO_ERROR_STREAM(x) std::cerr<<"\033[1;31m[ERROR] "<<x<<"\033[0;0m"<<std::endl;
#define SVO_ERROR_STREAM(x) std::cerr<<"*****[ERROR] "<<x<<"*****"<<std::endl;
