...
{
public:  
	//only use if this Frame is KeyFrame
	int	kf_id_;
	static int	kf_counter_;
  ...
  void UpdateNavState(const Preintegration& imupreint, const Eigen::Vector3d& gw);
  void UpdatePoseFromNS(const Eigen::Matrix4d &Tbc);

  Frame(vk::AbstractCamera* cam, const cv::Mat& img, double timestamp);
  ~Frame();
  void setNavState(const NavState& ns) { imuState = ns;}
  const Preintegration& getIMUPreInt() { return imuPreint; }
