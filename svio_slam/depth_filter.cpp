...
void DepthFilter::stopThread()
{
  ...
  if(thread_ != NULL)
  {
	...
	//this should be fixed since it's not safe
    //thread_->interrupt();
    thread_->join();
    thread_ = NULL;
  }
...
{
	//simply use joinable ,will fixed later
  while(true/*std::this_thread::interruption_requested()*/)
  {
    FramePtr frame;
    {
      lock_t lock(frame_queue_mut_);
...
void DepthFilter::updateSeeds(FramePtr frame)
{
  ...
  while( it!=seeds_.end())
  {
    ...
    const Eigen::Vector3d xyz_f(T_ref_cur.inverse()*(1.0/it->mu * it->ftr->f) );
    if(xyz_f.z() < 0.0)  {
    ...
    if(sqrt(it->sigma2) < it->z_range/options_.seed_convergence_sigma2_thresh)
    {
      ...
      Eigen::Vector3d xyz_world(it->ftr->frame->T_f_w_.inverse() * (it->ftr->f * (1.0/it->mu)));
...
void DepthFilter::updateSeed(const float x, const float tau2, Seed* seed)
{
  ...
  //use a simple normal distribution inst
  const float inv_sqrt_2pi = 0.3989422804014327;
  float a = (x - seed->mu) / norm_scale;
  float pdf = inv_sqrt_2pi / norm_scale * std::exp(-0.5f * a * a);

  float C1 = seed->a / (seed->a + seed->b) * pdf;//boost::math::pdf(nd, x);
