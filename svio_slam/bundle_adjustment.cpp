    ...
    Map* map)
{
  ...
  frame1->T_f_w_.so3().matrix() = v_frame1->estimate().rotation().toRotationMatrix();
  frame1->T_f_w_.translation() = v_frame1->estimate().translation();
  frame2->T_f_w_.so3().matrix() = v_frame2->estimate().rotation().toRotationMatrix();
    ...
    double& final_error)
{
  ...
  for(std::set<Point*>::iterator it_pt = mps.begin(); it_pt!=mps.end(); ++it_pt)
  {
    ...
    {
      Eigen::Vector2d error = vk::project2d((*it_obs)->f) - vk::project2d((*it_obs)->frame->w2f((*it_pt)->pos_));
...
void globalBA(Map* map)
{
      ...
      it_kf != map->keyframes_.end(); ++it_kf)
  {
    ...
    for(Features::iterator it_ftr=(*it_kf)->fts_.begin(); it_ftr!=(*it_kf)->fts_.end(); ++it_ftr)
    {
      ...
      Eigen::Vector2d error = vk::project2d((*it_ftr)->f) - vk::project2d((*it_kf)->w2f(mp->pos_));
