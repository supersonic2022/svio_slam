    ...
    Eigen::Matrix2d& A_cur_ref)
{
  ...
  Eigen::Vector3d xyz_du_ref(cam_ref.cam2world(px_ref + Eigen::Vector2d(halfpatch_size,0)*(1<<level_ref)));
  Eigen::Vector3d xyz_dv_ref(cam_ref.cam2world(px_ref + Eigen::Vector2d(0,halfpatch_size)*(1<<level_ref)));
  xyz_du_ref *= xyz_ref[2]/xyz_du_ref[2];
  xyz_dv_ref *= xyz_ref[2]/xyz_dv_ref[2];
  const Eigen::Vector2d px_cur(cam_cur.world2cam(T_cur_ref*(xyz_ref)));
  const Eigen::Vector2d px_du(cam_cur.world2cam(T_cur_ref*(xyz_du_ref)));
  const Eigen::Vector2d px_dv(cam_cur.world2cam(T_cur_ref*(xyz_dv_ref)));
...
bool depthFromTriangulation(
    ...
    double& depth)
{
  Eigen::Matrix<double,3,2> A; A << T_search_ref.so3().matrix() * f_ref, f_cur;
    ...
    double& depth)
{
  ...
  Eigen::Vector2d A = vk::project2d(T_cur_ref * (ref_ftr.f*d_min));
  Eigen::Vector2d B = vk::project2d(T_cur_ref * (ref_ftr.f*d_max));
  epi_dir_ = A - B;
  ...
  Eigen::Vector2d px_A(cur_frame.cam_->world2cam(A));
  Eigen::Vector2d px_B(cur_frame.cam_->world2cam(B));
  ...
  {
    Eigen::Vector2d px(cur_frame.cam_->world2cam(uv));
