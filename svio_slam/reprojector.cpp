...
{
  grid_.cell_size = Config::gridSize(); //default = 25
    ...
    std::vector< std::pair<FramePtr,std::size_t> >& overlap_kfs)
{
	...
	//the second double is the translation
	std::list< std::pair<FramePtr,double> > close_kfs;
	map_.getCloseKeyframes(frame, close_kfs);

	// Sort KFs with overlap according to their closeness
	close_kfs.sort(std::bind(
				 std::less<double>(),
				 std::bind(&std::pair<FramePtr, double>::second, std::placeholders::_1),
                 std::bind(&std::pair<FramePtr, double>::second, std::placeholders::_2)));
...
{
  cell.sort(std::bind(&Reprojector::pointQualityComparator, std::placeholders::_1, std::placeholders::_2));
...
{
  Eigen::Vector2d px(frame->w2c(point->pos_));
