...
Preintegration::Preintegration(const Preintegration& pre) :
...
void Preintegration::update(const Eigen::Vector3d& omega_true, const Eigen::Vector3d& acc_true, const double& dt)
{
	double dt2 = dt*dt;

	Eigen::Matrix3d dR = Expmap(omega_true * dt);
	Eigen::Matrix3d Jr = JacobianR(omega_true * dt);
	...
	A.block<3, 3>(3, 6) = -_delta_R*skew(acc_true)*dt;
	A.block<3, 3>(0, 6) = -0.5*_delta_R*skew(acc_true)*dt2;
	...
	_J_P_Biasg += _J_V_Biasg*dt - 0.5*_delta_R*skew(acc_true)*_J_R_Biasg*dt2;
	_J_V_Biasa += -_delta_R*dt;
	_J_V_Biasg += -_delta_R*skew(acc_true)*_J_R_Biasg*dt;
	...
	_delta_P += _delta_V*dt + 0.5*_delta_R*acc_true*dt2;    // P_k+1 = P_k + V_k*dt + R_k*a_k*dt*dt/2
	_delta_V += _delta_R*acc_true*dt;
