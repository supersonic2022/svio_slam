...
Point::Point(const Eigen::Vector3d& pos, Feature* ftr) :
  id_(point_counter_++),
  pos_(pos),
  normal_set_(false),
  n_obs_(1),
...
void Point::initNormal()
{
  ...
  normal_ = ftr->frame->T_f_w_.so3().matrix().transpose()*(-ftr->f);
  normal_information_ = DiagonalMatrix<double,3,3>(pow(20/(pos_-ftr->frame->pos()).norm(),2), 1.0, 1.0);
  normal_set_ = true;
}

bool Point::getCloseViewObs(const Eigen::Vector3d& framepos, Feature*& ftr) const
{
  ...
  for(auto it=obs_.begin(), ite=obs_.end(); it!=ite; ++it)
  {
    Eigen::Vector3d dir((*it)->frame->pos() - pos_); dir.normalize();
...
void Point::optimize(const size_t n_iter)
{
  ...
  for(size_t i=0; i<n_iter; i++)
  {
    ...
    {
      Matrix23d J;
      const Eigen::Vector3d p_in_f((*it)->frame->T_f_w_ * pos_);
      Point::jacobian_xyz2uv(p_in_f, (*it)->frame->T_f_w_.so3().matrix(), J);
      const Eigen::Vector2d e(vk::project2d((*it)->f) - vk::project2d(p_in_f));
