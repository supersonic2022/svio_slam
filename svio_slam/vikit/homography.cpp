...
calcFromPlaneParams(const Eigen::Vector3d& n_c1, const Eigen::Vector3d& xyz_c1)
{
  ...
  H_c2_from_c1 = T_c2_from_c1.so3().matrix() + (T_c2_from_c1.translation()*n_c1.transpose())/d;
}
...
findBestDecomposition()
{
  ...
  for(size_t i=0; i<decompositions.size(); i++)
  {
    ...
    decom.score = -static_cast<int>(nPositive);
  }
  ...
  else  // two-way ambiguity. Resolve by sampsonus score of all points.
  {
    ...
    for(size_t i=0; i<2; i++)
    {
      ...
      Eigen::Matrix3d Essential = T.so3().matrix() * sqew(T.translation());
      double dSumError = 0;
      for(size_t m=0; m < fts_c1.size(); m++ )
      {
