...
double computeInliers(
    const vector<Eigen::Vector3d>& features1,
    const vector<Eigen::Vector3d>& features2,
    const Eigen::Matrix3d& R,
    const Eigen::Vector3d& t,
...
void computeInliersOneView(
    const vector<Eigen::Vector3d> & feature_sphere_vec,
...
Quaterniond angax2quat(const Eigen::Vector3d& n, const double& angle);
